...
// fetchPoolsFromPage 从网页获取池子数据（备用方法）：三条链分开请求
func (s *kyberSwapImpl) fetchPoolsFromPage(ctx context.Context, page int) ([]model.Pool, error) {
	...
	for _, chainID := range earnServiceChainIDs {
		url := fmt.Sprintf("https://kyberswap.com/earn/pools?tag=high_apr&chainIds=%d&page=%d", chainID, page)
		pools, err := s.fetchPoolsFromHTMLURL(ctx, url)
		if err != nil {
			continue
		}
...
func (s *kyberSwapImpl) fetchPoolsFromHTMLURL(ctx context.Context, url string) ([]model.Pool, error) {
